...
<style>
    ...
    #myMap {
        height: 100vh; /* La carte occupe toute la hauteur de la page */
        width: 100%;
    }
</style>
@endsection
...
<script type="module">
    ...
    window.initMap = async function() {
        ...
        const map = new google.maps.Map(document.getElementById("myMap"), {
            center: { lat: 6.5, lng: 2.5 }, // Centre sur le sud du Bénin
        ...
        rides.forEach(async ride => {
            if (ride.latitude && ride.longitude) {
                ...
                const marker = new google.maps.Marker({
                    position: position,
                    map: map,
                    title: `Trajet ID: ${ride.numero}`,
                    icon: {
                        url: "{{ asset('taxi-solid.svg') }}",  // Chemin vers l'icône SVG
                        scaledSize: new google.maps.Size(30, 30),  // Ajuster la taille de l'icône
                        origin: new google.maps.Point(0, 0),
                        anchor: new google.maps.Point(15, 15),  // Point d'ancrage pour centrer l'icône
                        fillColor: "#FF6A13",  // Couleur de l'icône (orange foncé)
                        fillOpacity: 1,
                    }
                });
                ...
                const infoWindow = new google.maps.InfoWindow({
                    content: `<div>
                                <strong>Conducteur:</strong> ${ride.driver_name} </br>
                                <strong>Tél:</strong> ${ride.phone}
                        </div>`, // Affiche le nom du conducteur
                });

                // Lier l'infobulle au marqueur
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        script.src = "https://maps.googleapis.com/maps/api/js?key=AIzaSyCBteeTZAzfzhLkWeucdSI--ReIT_GLpmQ&callback=initMap";
        script.async = true;
        script.defer = true;
